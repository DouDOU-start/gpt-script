...
func NewPool(cfg PoolConfig) *Pool {
	stageFlags := map[Stage]bool{StageRegister: true, StageOAuth: true, StageTeam: true, StageSub2API: true}
...
func scopeForStage(stage Stage) string {
	switch stage {
	case StageRegister, StageOAuth, StageTeam, StageSub2API:
		return "account"
	default:
		return string(stage)
	}
