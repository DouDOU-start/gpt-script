...
func appendTimeline(path string, event map[string]any) error {
	...
	line := fmt.Sprintf("%s [%s] %s", timestamp, stage, status)
	if detail != "" {
		line += " - " + detail
	}
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
...
func timestampNow() string {
	return time.Now().In(time.FixedZone("CST", 8*3600)).Format("2006-01-02T15:04:05.000000+08:00")
}

func deepMerge(dst map[string]any, updates map[string]any) {
