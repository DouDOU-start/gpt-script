...
func (r *ChatGPTRegister) inputPassword() error {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		...
		state = r.waitForPostPasswordState(2 * time.Second)
		if isPostPasswordState(state) {
			log.Printf("[%s] 密码提交后页面已进入后续状态，继续流程: %s", r.taskID, state)
			return nil
		}
		if attempt < 3 {
...
func (r *ChatGPTRegister) waitForPostPasswordState(timeout time.Duration) RegisterPageState {
	deadline := time.Now().Add(timeout)
	last := RegisterStateUnknown
	for time.Now().Before(deadline) {
		state := detectRegisterState(r.browser, r.email)
		last = state
		if state != RegisterStatePassword && state != RegisterStateUnknown {
			return state
		}
		time.Sleep(250 * time.Millisecond)
	}
...
func isBrowserContextLostError(err error) bool {
	...
	return strings.Contains(message, "Cannot find context with specified id") ||
		...
		strings.Contains(message, "cannot find object") ||
		strings.Contains(message, "Could not find object with given id")
}
