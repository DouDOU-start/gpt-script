...
type YamlConfig struct {
	...
	IncludeSecrets  *bool        `yaml:"include_secrets,omitempty" json:"-"`
...
func (s *YamlStore) Update(fn func(*YamlConfig) error) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (c YamlConfig) IncludeSecretsValue() bool {
	if c.IncludeSecrets == nil {
		return true
	}
	return *c.IncludeSecrets
}
...
func applyYamlDefaults(cfg *YamlConfig) {
	...
	if cfg.Proxy.ApplyToRegister == false && cfg.Proxy.ApplyToOAuth == false && cfg.Proxy.ApplyToTeam == false {
