	...
	for i := 0; i < workers; i++ {
		...
		go func(workerID int) {
			...
			for acc := range jobs {
				...
				if rc.accountStore != nil && rc.dbWriteResults {
					if err := rc.accountStore.UpdateFromResult(ctx, dbResultUpdate(result, rc.runID)); err != nil {
						log.Printf("[worker-%d] %s 数据库回写失败: %v", workerID, acc.Email, err)
					}
				}
				if result.Success {
...
func dbResultUpdate(result runResult, runID string) accountstore.ResultUpdate {
	return accountstore.ResultUpdate{
		Email:             result.Email,
		Mode:              result.Mode,
		...
		OAuthPlanType:     result.OAuthPlanType,
