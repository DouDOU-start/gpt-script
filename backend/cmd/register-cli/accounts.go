...
func loadAccounts(email, password, path string) ([]accountInput, error) {
	...
	for scanner.Scan() {
		...
		acc, ok, err := parseAccountLine(line)
		if err != nil {
			return nil, err
		}
		if !ok {
...
func loadAccountsFromDB(ctx context.Context, store accountstore.Store, statuses []string, limit int) ([]accountInput, error) {
	accounts, err := store.ListAccounts(ctx, accountstore.Query{Statuses: statuses, Limit: limit})
	if err != nil {
		return nil, err
	}
...
func saveGeneratedAccountsToDB(ctx context.Context, store accountstore.Store, accounts []accountInput) error {
	for _, acc := range accounts {
		if _, err := store.UpsertAccount(ctx, accountstore.Account{Email: acc.Email, Password: acc.Password, Status: "active"}); err != nil {
			return err
		}
	}
	return nil
...
func parseAccountLine(line string) (accountInput, bool, error) {
	if strings.HasPrefix(line, "{") {
		var result runResult
		if err := json.Unmarshal([]byte(line), &result); err != nil {
			return accountInput{}, false, fmt.Errorf("解析账号 JSON 失败: %w", err)
		}
		if result.TaskID != "" && !result.Success {
			return accountInput{}, false, nil
		}
		acc := accountInput{Email: strings.TrimSpace(result.Email), Password: result.Password}
		if acc.Email == "" || acc.Password == "" {
			return accountInput{}, false, fmt.Errorf("账号 JSON 缺少 email/password")
		}
		return acc, true, nil
	}
	...
	for _, sep := range []string{"----", ",", "\t"} {
		...
		if len(parts) == 2 {
			...
			if acc.Email == "" || acc.Password == "" {
				return accountInput{}, false, fmt.Errorf("账号行缺少邮箱或密码: %s", line)
			}
			return acc, true, nil
		}
	}
	return accountInput{}, false, fmt.Errorf("账号行格式错误: %s", line)
}
